...
def create_bar_chart(data):
    fig = go.Figure(go.Bar(
        ...
        y=[item['percentual'] for item in data],
        marker_color='#00A9FF'
    ))
    fig.update_layout(
        title="Visão Geral",
...
def create_radar_chart(data):
    ...
    fig.add_trace(go.Scatterpolar(
        r=[item['percentual'] for item in data],
...
def show_welcome_page():
    ...
    st.markdown("""
    Descubra o nível de maturidade digital do seu negócio em **Branding, Marketing e Vendas**.
    Responda a 15 perguntas rápidas e receba um diagnóstico completo com insights para acelerar seus resultados.
...
def show_results_page():
    ...
    for cat, questions in DIAGNOSTIC_QUESTIONS.items():
        ...
        for i, q in enumerate(questions):
            cat_score += st.session_state.answers.get(f"{cat}-{i}", 0)
        
        scores[cat] = cat_score
        total_possible[cat] = len(questions) * 3
    ...
    for i, cat in enumerate(percentages):
        with cols[i]:
            ...
            if score <= 30:
                ...
                recommendation = 'Atenção! Sua estratégia digital nesta área está vulnerável. É crucial agir agora para não perder mais oportunidades.'
            elif score <= 70:
                level = 'Ponto de Melhoria'
                recommendation = 'Você já deu os primeiros passos, mas existem lacunas que limitam seu crescimento. É o momento ideal para profissionalizar suas ações.'
            else:
                level = 'Estágio Acelerado'
                recommendation = 'Sua estratégia está bem direcionada. O desafio agora é manter a consistência e buscar otimizações contínuas.'
    ...
    with col1:
        st.plotly_chart(create_bar_chart(chart_data), use_container_width=True)
    with col2:
        st.plotly_chart(create_radar_chart(chart_data), use_container_width=True)
    ...
    if st.session_state.get('show_form', False):
        with st.form("whatsapp_form"):
            ...
            st.write("Para onde posso enviar o resumo da nossa conversa para você consultar depois?")
            ...
            if submit_button:
                if name and phone:
                    analysis_text = f"*Diagnóstico de Maturidade Digital para {name}*\n\n"
                    analysis_text += "Olá! Segue o resumo da sua análise, como combinado. Salve esta mensagem para consultar depois.\n\n"
                    for cat, perc in percentages.items():
                         analysis_text += f"*{cat.upper()}:* {perc}%\n"
                    analysis_text += "\n*Próximos Passos:*\nAnalisando seus resultados, podemos traçar um plano de ação prático. Quando podemos conversar por 15 minutos?\n\nLembre-se: Seu negócio é a única opção ou apenas mais uma?"
                    ...
                    <div style="text-align: center; margin-top: 20px;">
                        <p>Perfeito! Preparei o seu diagnóstico. Como não salvamos seus dados por privacidade, clique no link abaixo para abrir o seu WhatsApp e salvar sua análise agora mesmo:</p>
                        <a href="{whatsapp_url}" target="_blank" style="display: inline-block; padding: 15px 25px; font-size: 18px; color: white; background-color: #25D366; border-radius: 30px; text-decoration: none; font-weight: bold;">
                            📲 CLIQUE AQUI PARA SALVAR MINHA ANÁLISE
                        </a>
                    </div>
                    """
